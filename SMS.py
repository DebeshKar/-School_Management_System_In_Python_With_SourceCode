...
class SchoolManagementSystem:
    ...
    def show_main_menu(self):
        ...
        tk.Button(scrollable_frame, text="Add Hosteler", command=self.show_add_hosteler, font=("Arial", 12)).pack(
            pady=10)
        tk.Button(scrollable_frame, text="Add Bus Holder", command=self.show_add_bus_holder, font=("Arial", 12)).pack(
            pady=10)
        tk.Button(scrollable_frame, text="Add Employee", command=self.show_add_employee, font=("Arial", 12)).pack(
            pady=10)
        tk.Button(scrollable_frame, text="Record Fee Payment", command=self.show_fee_payment, font=("Arial", 12)).pack(
            pady=10)
        tk.Button(scrollable_frame, text="Generate Salary Slip", command=self.show_salary_slip,
                  font=("Arial", 12)).pack(pady=10)
        tk.Button(scrollable_frame, text="Generate Report Card", command=self.show_report_card,
                  font=("Arial", 12)).pack(pady=10)
        tk.Button(scrollable_frame, text="Search Students", command=self.show_search_students, font=("Arial", 12)).pack(
            pady=10)
        tk.Button(scrollable_frame, text="Generate No Dues", command=self.show_no_dues, font=("Arial", 12)).pack(
            pady=10)
        tk.Button(scrollable_frame, text="Change Password", command=self.show_change_password, font=("Arial", 12)).pack(
            pady=10)
        tk.Button(scrollable_frame, text="Backup Database", command=self.backup_database, font=("Arial", 12)).pack(
            pady=10)
        tk.Button(scrollable_frame, text="Restore Database", command=self.restore_database, font=("Arial", 12)).pack(
            pady=10)
    ...
    def save_student(self):
        ...
        c = conn.cursor()
        c.execute(
            "INSERT INTO students (student_id, name, class, hostel_status, bus_status, created_at) VALUES (?, ?, ?, ?, ?, ?)",
    ...
    def save_fee_payment(self):
        ...
        c.execute(
            "INSERT INTO fee_transactions (transaction_id, student_id, fee_type, amount, payment_date) VALUES (?, ?, ?, ?, ?)",
